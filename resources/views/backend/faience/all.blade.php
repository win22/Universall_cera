    ...
    <p hidden>   {{ \Carbon\Carbon::setLocale('fr') }}</p>
        ...
        <div style="background-color: white" class="card-header">
            <div class="float-left">
                ...
                <p>Catégorie: Faiences</p>
            </div>
            <div style="padding-top: 20px" class="float-right">
                <form class="form-inline ml-3" action="{{ route('sear_faience') }}" method="post">
                    @csrf
        ...
        <div class="card-body">
        <p>Vous etes dans la catégorie faience dans l'option céramique </p>
            <div  class="shadow table-responsive  card-success1 card-outline1">
                <table class="table table-hover">
                    ...
                    <tr>
                        ...
                        <td>
                            <a href="{{ route('detail_faience',array('test' => $v_info->id)) }}">{{ $v_info['model'] }}</a></td>
                        <td>{{ $v_info['taille'] }}</td>
                        <td>
                            {{ $v_info['prix'] }} FCFA
                        </td>
                        ...
                        <td class="td-actions text-center">
                            ...
                            @if($v_info->status == 1)
                            <a href="/desactive_faience/{{$v_info['id']}}" class="btn btn-dark btn-link btn-sm">
                                <i style="color: white !important;" class="fas fa-thumbs-down"></i>
                            </a>
                            @else
                            <a href="/active_faience/{{$v_info['id']}}" class="btn btn-info btn-link btn-sm">
                            ...
                            <a id="delete" href="/delete_faience/{{ $v_info['id'] }}"
            ...
            <div style="padding-top: 30px" class="card-tools">
                <a href="#" class="btn btn-info  btn-sm" data-toggle="modal" data-target="#exampleModal">
                    <i style="color: #ffffff !important;" class="fas fa-plus"></i> &nbsp; Ajouter un nouveau faience</a>
    ...
    <div class="modal-dialog modal-lg " role="document">
        <div class="modal-content card-orange card-outline">
            <div class="modal-header">
                <h5 class="modal-title" id="exampleModalLabel">Ajouter un produit de type faience</h5>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
            <div class="modal-body">
                <form enctype="multipart/form-data" action="{{ route('faience_save') }}" method="post">
                    @csrf
                    <div class="row">
